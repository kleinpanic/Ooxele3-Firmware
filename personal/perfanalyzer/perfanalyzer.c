...
static void perf_draw(Canvas* canvas, void* ctx) {
    PerfAnalyzerState* st = ctx;
    furi_mutex_acquire(st->mutex, FuriWaitForever);
    canvas_clear(canvas);
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str(canvas, 8, 16, "PerfAnalyzer");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 8, 36, "App profiler");
    canvas_draw_str(canvas, 8, 52, "Press BACK to exit");
    furi_mutex_release(st->mutex);
}

static void perf_input(InputEvent* event, void* ctx) {
...
int32_t perfanalyzer_app(void* p) {
    UNUSED(p);
    FURI_LOG_I("perfanalyzer", "starting app profiler");
    ...
    view_port_draw_callback_set(vp, perf_draw, &state);
    view_port_input_callback_set(vp, perf_input, q);
