...
static void proc_draw(Canvas* canvas, void* ctx) {
...
static void proc_input(InputEvent* event, void* ctx) {
...
int32_t processissues_app(void* p) {
    UNUSED(p);
    FURI_LOG_I("processissues", "starting crash tracker");
    ...
    view_port_draw_callback_set(vp, proc_draw, &state);
    view_port_input_callback_set(vp, proc_input, q);
