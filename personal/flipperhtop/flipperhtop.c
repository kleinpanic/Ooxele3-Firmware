...
static void ftop_draw(Canvas* canvas, void* ctx) {
    FlipperHtopState* st = ctx;
    furi_mutex_acquire(st->mutex, FuriWaitForever);
    canvas_clear(canvas);
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str(canvas, 8, 16, "FlipperHtop");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 8, 36, "Thread Monitor");
    canvas_draw_str(canvas, 8, 52, "Press BACK to exit");
    furi_mutex_release(st->mutex);
}

static void ftop_input(InputEvent* event, void* ctx) {
...
int32_t flipperhtop_app(void* p) {
    UNUSED(p);
    FURI_LOG_I("flipperhtop", "starting thread monitor");
    ...
    view_port_draw_callback_set(vp, ftop_draw, &state);
    view_port_input_callback_set(vp, ftop_input, q);
